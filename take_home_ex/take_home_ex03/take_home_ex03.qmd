...
```{r}
pacman::p_load(olsrr, corrplot, sf, spdep, tmap, tidyverse, gtsummary, sfdep, onemapsgapi, httr, rjson, plotly, ggplot2, GGally, units, matrixStats)
```
...
```{r}
#| code-fold: true
c = 1007500 #quantile(resale$resale_price, .95) + 1.5 * IQR(resale$resale_price)
cat("Top 5% of resale prices are over -", c)
```

These outliers can induce a bias so the data points of the top 5% resale prices (prices over \$1,007,500) will be removed.
...
The storey_range is a categorical variable with 16 categories - "01 TO 03", "04 TO 06" upto "46 TO 48". Keeping them as separate categories would induce high carnality into the model.
...
Based on an initial glance, higher floor_area_sqm seems to increase resale price to a certain extent, remaining_lease directly increases resale_price, and higher storey_num increases resale_price significantly when it's in the 13-48 storey range.
...
| 322 YISHUN CTRL         | 760322 |
| 361 HOUGANG AVE 5       | 310083 |
| 386 BT BATOK WEST AVE 5 | 650386 |
| 407 YISHUN AVE 6        | 760407 |
| 447 CHOA CHU KANG AVE 4 | 680447 |
| 620 WOODLANDS DR 52     | 730620 |
| 639 PASIR RIS DR 1      | 510639 |
...
#| eval: false
#| code-fold: true
indices = c(786, 1331, 1339, 1405, 1920, 2216, 2434, 2760, 2833, 2951, 3357, 4429, 4578)
postal_codes = c("530154", "680215", "680216", "550220", "520273", "400304", "760322", "310083", "650386", "760407", "680447", "730620", "510639")
...
rs_coords <- subset(rs_coords, 
                    select = c(month, storey_num, floor_area_sqm, remaining_lease, resale_price))
```
...
```{r}
# resale <- get_prox(resale, kindergartens, "PROX_Kindergartens") 
```

```{r}
# resale <- proximity(resale, cbd, "PROX_CBD")
# 
# resale <- proximity(resale, eldercare, "PROX_ELDERCARE")
#
# resale <- proximity(resale, hawker_centers, "PROX_HAWKER_CENTRES")
# 
# resale <- proximity(resale, childcare, "PROX_CHILDCARE")
# 
# resale <- proximity(resale, kindergartens, "PROX_KINDERGARTENS")
# 
# resale <- proximity(resale, parks, "PROX_PARKS")
