...
all_dates <- list()
for (file in files) {
  all_dates <- append(all_dates, str_extract(file, "\\d{2} [[:alpha:]]+ \\d{4}"))
}
...
#Alternatively can use fisher instead of jenks

plot<-function(dt) {
  tm_shape(filter(df, dmy(dt) %in% df$Date)) +
    tm_fill("vaccination_rate",
            n= 6,
            style = "jenks",
            palette="RdPu")+
    tm_borders(alpha = 0.5) +
    tm_layout(main.title= paste("Vaccination rate by", substring(dt,4,nchar(dt))),
              frame=TRUE)+
      tm_grid(alpha=0.2)
}
```
